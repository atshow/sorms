...
public class ArrayUtils {
    ...
    public static <T> T[] toArray(final List<T> items) {
        if (items != null) {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> T[] toArray(final List<T> items, Class<T> clz) {
        if (items != null) {
            Object o = Array.newInstance(clz, items.size());//初始化一个长度为3的对应类型的数组
            int len = Array.getLength(o);//获取初始化的数组长度
            for (int i = 0; i < len; ++i) {//对数组对象依次赋值
                Array.set(o, i, items.get(i));
            }
            return (T[]) o;
        } else {
            return null;
